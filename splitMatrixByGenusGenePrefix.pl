...
my ($level,$genus_list,$infile);

Usage("Too few arguments") if $#ARGV < 0;
GetOptions( "h|?|help" => sub { &Usage(); },
            "l|level=s"=> \$level,
            "i|infile=s"=>\$infile,
            "g|genus=s"=>\$genus_list
    ) or &Usage();
...
$LOGGER->logdie("Missing genus/genera") unless ($genus_list);

my $re_pattern;
$genus_list=~s/^\s+//;
$genus_list=~s/\s+$//;
$genus_list=~s/\s+//g;
my @genus = split(/,/, $genus_list);

my $genus_pattern=join("|", @genus);

$re_pattern=qr/^(?:$genus_pattern)_/;
...
Argument(s)
        ...
        -h      --help      Help
        -l      --level     Log level [Default: FATAL]
        -i      --infile    Input file (Gene Expression Matrix with Genus prefix for genes)
        -g      --genus     Genus or comma separated list of genera
